...
async fn test_basic_put_get_delete() {
    ...
    // Put
    engine.put(key.clone(), val.clone()).await.unwrap();
...
async fn test_crash_recovery_via_wal_replay() {
    ...
    {
        let engine = ApexEngine::open(dir.path()).unwrap();
        engine.put(key1.clone(), val1.clone()).await.unwrap();
        engine.put(key2.clone(), val2.clone()).await.unwrap();
        engine.delete(key1.clone()).await.unwrap();
...
async fn test_overwrite_survives_recovery() {
    ...
    {
        let engine = ApexEngine::open(dir.path()).unwrap();
        engine.put(key.clone(), Bytes::from("1")).await.unwrap();
        engine.put(key.clone(), Bytes::from("2")).await.unwrap();
        engine.put(key.clone(), Bytes::from("3")).await.unwrap();
    }
            ...
            for op in ops {
                match op {
                    Op::Put(k, v) => {
                        ...
                        engine.put(key, val.clone()).await.unwrap();
                        model.insert(k, val);
                    }
                    Op::Delete(k) => {
                        let key = Bytes::from(k.clone());
