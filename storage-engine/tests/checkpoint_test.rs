...
mod tests {
    ...
    async fn test_checkpoint_consistency() {
        ...
        // 1. Write some data
        engine.put(Bytes::from("key1"), Bytes::from("val1")).await.unwrap();
        engine.put(Bytes::from("key2"), Bytes::from("val2")).await.unwrap();
        ...
        engine.put(Bytes::from("key3"), Bytes::from("val3")).await.unwrap();
        ...
        let has_sst = fs::read_dir(&cp_path).unwrap().any(|e| e.unwrap().file_name().to_str().unwrap().ends_with(".sst"));
        let has_wal = fs::read_dir(&cp_path).unwrap().any(|e| e.unwrap().file_name().to_str().unwrap().ends_with(".wal"));
        ...
        engine.put(Bytes::from("key4"), Bytes::from("val4")).await.unwrap();
