...
use crate::validation::adversarial::core::event_log::{EventLog, AdversarialEvent};
...
impl AdversarialNetworkConnection {
    async fn apply_faults(&self, msg_type: &str) -> Result<Option<Duration>, RPCError<u64, BasicNode, RaftError<u64>>> {
        ...
        if action.drop {
            ...
            return Err(RPCError::Network(NetworkError::new(&std::io::Error::new(std::io::ErrorKind::ConnectionAborted, "Dropped by Interceptor"))));
        }
...
impl RaftNetwork<ApexRaftTypeConfig> for AdversarialNetworkConnection {
    ...
    async fn install_snapshot(
        ...
        match self.apply_faults("InstallSnapshot").await {
            Ok(_) => {},
            Err(_) => return Err(RPCError::Network(NetworkError::new(&std::io::Error::new(std::io::ErrorKind::ConnectionAborted, "Dropped by Interceptor")))),
        }
