...
async fn adversarial_reordering_safety() -> Result<()> {
    ...
    harness.log.log(AdversarialEvent::TestInfo(
        "Starting Reordering Safety Test".to_string(),
    ));
    ...
    harness.log.log(AdversarialEvent::TestInfo(
        "Applied 10ms-500ms random jitter to all links".to_string(),
    ));

    harness.log.log(AdversarialEvent::TestInfo(
        "Issuing concurrent writes under jitter...".to_string(),
    ));
    ...
    for i in 0..10 {
        ...
        tasks.push(tokio::spawn(
            async move { h.inner.put(leader, key, val).await },
        ));
    }
    ...
    harness.log.log(AdversarialEvent::TestInfo(
        "All writes completed".to_string(),
    ));

    tokio::time::sleep(Duration::from_secs(2)).await;

    harness.log.log(AdversarialEvent::TestInfo(
        "Verifying final consistency...".to_string(),
    ));
    for i in 0..10 {
        let key = format!("reorder_key_{}", i).into_bytes();
        ...
        for &id in harness.inner.live.keys() {
            ...
            if first_val.is_none() {
                ...
                assert_eq!(
                    first_val,
                    Some(val),
                    "Divergence detected for key reorder_key_{} on node {}",
                    i,
                    id
                );
            }
    ...
    harness.log.log(AdversarialEvent::TestInfo(
        "Reordering safety verified".to_string(),
    ));
    Ok(())
}
