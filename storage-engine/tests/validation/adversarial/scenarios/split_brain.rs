...
async fn adversarial_split_brain_survival() -> Result<()> {
    ...
    // Verify membership
    let m = harness.inner.live[&leader]
        .node
        .raft
        .metrics()
        .borrow()
        .clone();
    harness.log.log(AdversarialEvent::TestInfo(format!(
        "Leader {} metrics: membership={:?}, state={:?}",
        leader, m.membership_config, m.state
    )));
    assert_eq!(
        m.membership_config.voter_ids().count(),
        3,
        "Cluster must have 3 voters"
    );
    ...
    harness.log.log(AdversarialEvent::TestInfo(format!(
        "Partitioned leader {} from followers {:?}",
        leader, followers
    )));

    // Attempt to write to the partitioned leader
    harness.log.log(AdversarialEvent::TestInfo(
        "Attempting write to partitioned leader (should fail to commit)".to_string(),
    ));
    ...
    assert!(
        write_res.is_err() || write_res.unwrap().is_err(),
        "Write to partitioned leader must not succeed"
    );
    harness.log.log(AdversarialEvent::TestInfo(
        "Confirmed: Write to partitioned leader did not commit".to_string(),
    ));

    // Wait for the majority to elect a new leader
    harness.log.log(AdversarialEvent::TestInfo(
        "Waiting for majority to elect new leader".to_string(),
    ));
    let new_leader = harness
        .inner
        .wait_for_new_leader(leader, Duration::from_secs(15))
        .await?;
    harness.log.log(AdversarialEvent::TestInfo(format!(
        "New leader elected: {}",
        new_leader
    ...
    harness
        .log
        .log(AdversarialEvent::TestInfo("Partition healed".to_string()));

    // Verify consistency: New leader can write and all nodes eventually see it
    harness.log.log(AdversarialEvent::TestInfo(
        "Verifying consistency across all nodes".to_string(),
    ));
    let key2 = b"post_heal_key".to_vec();
    let val2 = b"val2".to_vec();
    harness
        .inner
        .put(new_leader, key2.clone(), val2.clone())
        .await?;

    // Get the commit index from the leader to wait for it on others
    let leader_metrics = harness.inner.live[&new_leader]
        .node
        .raft
        .metrics()
        .borrow()
    ...
    harness.log.log(AdversarialEvent::TestInfo(
        "Split brain survival verified".to_string(),
    ));
    Ok(())
}
