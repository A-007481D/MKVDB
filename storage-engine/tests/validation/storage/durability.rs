...
async fn bootstrap_cluster(harness: &ClusterHarness) -> Result<u64> {
    ...
    members.insert(1u64, openraft::BasicNode { addr: harness.persistence[&1].bind_addr.clone() });
    node1.raft.initialize(members).await?;
    let leader = harness.wait_for_leader(Duration::from_secs(10)).await?;
    
    node1.raft.add_learner(2, openraft::BasicNode { addr: harness.persistence[&2].bind_addr.clone() }, true).await?;
    node1.raft.add_learner(3, openraft::BasicNode { addr: harness.persistence[&3].bind_addr.clone() }, true).await?;
...
async fn snapshot_and_restart_durability() -> Result<()> {
    ...
    eprintln!("[test] All entries written. Last index: {}. Waiting for convergence...", last_index);

    for &id in harness.live.keys() {
        harness.wait_for_applied(id, last_index, Duration::from_secs(15)).await?;
    }
    ...
    assert!(m_leader.snapshot.is_some(), "Snapshot should have been triggered by 1500 entries");
    eprintln!("[test] Snapshot confirmed at index: {:?}", m_leader.snapshot);
    ...
    for i in (0..1500).step_by(100) { // Check every 100th key to be efficient
        let key = format!("key_{}", i).into_bytes();
        let expected_val = format!("val_{}", i).into_bytes();
        
        let val = harness.get(new_leader, &key).await?;
        assert_eq!(val, Some(expected_val), "Data mismatch after restart at index {}", i);
    }
